...
from usd_exchange.serializers import USDExchangeSerializer


class USDExchangeView(generics.ListAPIView):
	...
	def get_queryset(self):
		...
		if date_from and date_to:
			return USDExchange.objects.filter(date__range=(date_from, date_to))
		raise Exception('query params null')

	def list(self, *args, **kwargs):
		try:
			queryset = self.get_queryset()
			serializer = USDExchangeSerializer(queryset, many=True)
